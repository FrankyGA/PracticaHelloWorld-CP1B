...
class TestApi(unittest.TestCase):
    ...
    def test_api_divide_by_zero(self):
        ...
        response = urlopen(url, timeout=DEFAULT_TIMEOUT)
        self.assertEqual(response.status, 406, f"Error en la petición API a {url}")
        self.assertEqual(response.read().decode(), "Division by zero is not possible", "ERROR DIVIDE BY ZERO")
